 ...
 * so it must never be pulled into a client component.
 *
 * Two reasons every Kite call is proxied through our own routes rather than made
 * from the browser: the session exchange needs the secret, and api.kite.trade
 * sends no CORS headers, so a direct fetch from the page would fail anyway.
 */
